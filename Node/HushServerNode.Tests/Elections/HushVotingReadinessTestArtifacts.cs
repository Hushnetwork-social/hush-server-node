...
internal static class HushVotingReadinessTestArtifacts
{
    ...
    public static VoidDecisionReadinessPromotionPaths CreateVoidDecisionReadinessPaths() =>
        VoidDecisionReadinessPromotionPaths.FromWorkspaceRoot(SharedWorkspaceRoot.Value);

    public static DisputeContinuityReadinessPromotionPaths CreateDisputeContinuityReadinessPaths() =>
        DisputeContinuityReadinessPromotionPaths.FromWorkspaceRoot(SharedWorkspaceRoot.Value);

    public static LegalGovernanceBoundaryPromotionPaths CreateLegalGovernanceBoundaryPaths() =>
        LegalGovernanceBoundaryPromotionPaths.FromWorkspaceRoot(SharedWorkspaceRoot.Value);

    public static GovernedOutcomeProducerPaths CreateGovernedOutcomeProducerPaths() =>
        GovernedOutcomeProducerPaths.FromWorkspaceRoot(SharedWorkspaceRoot.Value);
    ...
    public static PublicStateElectionPrerequisitePromotionPaths CreatePublicStateElectionPrerequisitePaths() =>
        PublicStateElectionPrerequisitePromotionPaths.FromWorkspaceRoot(SharedWorkspaceRoot.Value);
    ...
    public static string CreateEmptyWorkspace(string prefix)
    {
        ...
        Directory.CreateDirectory(Path.Combine(root, "hush-documents", "PrivateServer_ElectronicVoting", "Void-Decision-Publication-Replacement"));
        Directory.CreateDirectory(Path.Combine(root, "hush-documents", "PrivateServer_ElectronicVoting", "Dispute-Continuity-Readiness"));
        ...
        Directory.CreateDirectory(Path.Combine(root, "hush-documents", "PrivateServer_ElectronicVoting", "Public-State-Election-Prerequisites"));
        ...
        Directory.CreateDirectory(Path.Combine(root, "hush-memory-bank", "Overview", "HushVotingReadiness", "Public-State-Election-Prerequisites"));
    ...
    private static string CreateSharedWorkspaceRoot()
    {
        ...
        CopyDirectory(Path.Combine(FixtureRoot, "Verifier-Corpus"), Path.Combine(readinessRoot, "Verifier-Corpus"));
        CopyDirectory(Path.Combine(FixtureRoot, "Void-Decision-Publication-Replacement"), Path.Combine(readinessRoot, "Void-Decision-Publication-Replacement"));
        CopyDirectory(Path.Combine(FixtureRoot, "Dispute-Continuity-Readiness"), Path.Combine(readinessRoot, "Dispute-Continuity-Readiness"));
        ...
        CopyDirectory(Path.Combine(FixtureRoot, "Public-State-Election-Prerequisites"), Path.Combine(readinessRoot, "Public-State-Election-Prerequisites"));
        return root;
    }
