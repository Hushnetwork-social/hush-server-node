#!/usr/bin/env bash
# =============================================================================
# FEAT-013 focused qualification gate.
#
# Fails when:
#   - any configured FEAT-013 test block discovers ZERO tests, or
...
#
# Usage:
#   bash scripts/test-feat-013.sh             # unit + PostgreSQL TwinTests blocks
#   bash scripts/test-feat-013.sh --unit      # licensing unit/model tests only
#   bash scripts/test-feat-013.sh --twin      # licensing real-PostgreSQL TwinTests only
#   bash scripts/test-feat-013.sh --self-test # prove the zero-discovery guard fails non-zero
#
...
UNIT_PROJECT="$REPO_ROOT/Core/HushVoting/HushNode.HushVoting.Licensing.Storage.Tests/HushNode.HushVoting.Licensing.Storage.Tests.csproj"
TWIN_PROJECT="$REPO_ROOT/HushNode.IntegrationTests/HushNode.IntegrationTests.csproj"
TWIN_FILTER="Category=FEAT-013&Category=TwinTest&Category=NON_E2E"
...
if [[ "$failed" -ne 0 ]]; then
    echo "FEAT-013 GATE: FAILED (see counts above)."
    exit 1
fi

echo "FEAT-013 GATE: PASSED (unit + PostgreSQL TwinTests; zero-discovery guard armed)."
exit 0
