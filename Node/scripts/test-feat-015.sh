#!/usr/bin/env bash
# =============================================================================
# FEAT-015 focused qualification gate.
#
# Fails when:
#   - any configured FEAT-015 test block discovers ZERO tests, or
...
#
# Usage:
#   bash scripts/test-feat-015.sh             # all FEAT-015 blocks
#   bash scripts/test-feat-015.sh --unit      # licence transaction unit/contract only
#   bash scripts/test-feat-015.sh --grpc      # licence gRPC contract tests only
#   bash scripts/test-feat-015.sh --host      # host licence tests only
#   bash scripts/test-feat-015.sh --twin      # FEAT-015 real-PostgreSQL/Redis TwinTests only
#   bash scripts/test-feat-015.sh --self-test # prove the zero-discovery guard fails non-zero
#
...
UNIT_PROJECT="$REPO_ROOT/Core/HushVoting/HushNode.HushVoting.Licence.Transactions.Tests/HushNode.HushVoting.Licence.Transactions.Tests.csproj"
GRPC_PROJECT="$REPO_ROOT/Core/HushVoting/HushNode.HushVoting.Licence.gRPC.Tests/HushNode.HushVoting.Licence.gRPC.Tests.csproj"
HOST_PROJECT="$REPO_ROOT/HushServerNode.Tests/HushServerNode.Tests.csproj"
TWIN_PROJECT="$REPO_ROOT/HushNode.IntegrationTests/HushNode.IntegrationTests.csproj"
TWIN_FILTER="Category=FEAT-015&Category=TwinTest&Category=NON_E2E"
HOST_FILTER="FullyQualifiedName~HushVotingLicence"
...
if [[ "$mode" == "all" || "$mode" == "--unit" ]]; then
    run_block "unit-contract" "$UNIT_PROJECT" || failed=1
fi

if [[ "$mode" == "all" || "$mode" == "--grpc" ]]; then
    run_block "grpc-contract" "$GRPC_PROJECT" || failed=1
fi

if [[ "$mode" == "all" || "$mode" == "--host" ]]; then
    run_block "host-licence" "$HOST_PROJECT" --filter "$HOST_FILTER" || failed=1
fi

if [[ "$mode" == "all" || "$mode" == "--twin" ]]; then
    run_block "real-twin" "$TWIN_PROJECT" --filter "$TWIN_FILTER" || failed=1
fi

shutdown_build_servers

if [[ "$failed" -ne 0 ]]; then
    echo "FEAT-015 GATE: FAILED (see counts above)."
    exit 1
fi

echo "FEAT-015 GATE: PASSED (unit + gRPC + host + real TwinTest blocks; zero-discovery guard armed)."
exit 0
