...
public static class Program
{
    public static int Main(string[] args)
    {
        try
        {
            var workspaceRoot = GetOption(args, "--workspace-root", "-WorkspaceRoot", "-workspace-root") ?? WorkspaceRootFinder.Find(AppContext.BaseDirectory);
            ...
            var paths = DeploymentRollbackRehearsalPromotionPaths.FromWorkspaceRoot(workspaceRoot);
            var result = new DeploymentRollbackRehearsalPromotionService().Promote(new(
                paths,
                mode,
                sourceInput,
                outputRoot,
            ...
            Console.WriteLine($"Second ceremony root: {result.SecondCeremonyRoot}");
        ...
        catch (DeploymentRollbackRehearsalPromotionException ex)
        {
