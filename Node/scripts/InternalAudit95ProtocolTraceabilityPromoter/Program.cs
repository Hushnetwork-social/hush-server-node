...
public static class Program
{
    public static int Main(string[] args)
    {
        try
        {
            ...
            var paths = InternalAudit95ProtocolTraceabilityPaths.FromWorkspaceRoot(workspaceRoot);
            var result = new InternalAudit95ProtocolTraceabilityService().Promote(new(
                paths,
                mode,
                sourceInput,
                outputRoot,
            ...
            foreach (var blocker in result.GeneratedPackage.Blockers)
            {
            ...
            return result.Status == "blocked" && mode != InternalAudit95ProtocolTraceabilityService.ModeCheckOnly ? 1 : 0;
        }
        catch (InternalAudit95ProtocolTraceabilityException ex)
        {
