...
/// <summary>
/// Content handler for UnbanFromGroupFeed transactions.
/// Validates that:
/// - Sender is an admin in the group
/// - Target is currently banned
/// - Group exists and is not deleted
/// </summary>
public class UnbanFromGroupFeedContentHandler(
    ...
    : ITransactionContentHandler
{
    ...
    public bool CanValidate(Guid transactionKind) =>
        UnbanFromGroupFeedPayloadHandler.UnbanFromGroupFeedPayloadKind == transactionKind;

    public AbstractTransaction ValidateAndSign(AbstractTransaction transaction)
    {
        var signedTransaction = transaction as SignedTransaction<UnbanFromGroupFeedPayload>;
        ...
        // Validation: Target must exist and be Banned
        var targetParticipant = this._feedsStorageService
            .GetGroupFeedParticipantAsync(payload.FeedId, payload.UnbannedUserPublicAddress)
        ...
        {
            return null!; // Can only unban a banned member
        }
