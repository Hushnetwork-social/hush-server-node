...
public sealed partial class HushVotingPackageVerifier
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (File.Exists(ResolvePackagePath(request.PackagePath, VerificationPackageFileNames.FailedFinalizePackageManifest)))
        {
            return await VerifyFailedFinalizePackageAsync(request, cancellationToken);
        }
        ...
        try
        {
            ...
            results.AddRange(await CheckSp04EvidenceAsync(
                ...
                request.VerifierProfileId,
                cancellationToken));
            results.AddRange(await CheckSp05EvidenceAsync(
                request.PackagePath,
                manifest,
                request.VerifierProfileId,
                cancellationToken));
            results.AddRange(await CheckSp06EvidenceAsync(
                request.PackagePath,
                manifest,
                request.VerifierProfileId,
                cancellationToken));
            ...
            results.AddRange(await CheckSp07PublicationProofEvidenceAsync(
                ...
                cancellationToken));
            results.AddRange(await CheckReleaseIntegrityEvidenceAsync(
                request.PackagePath,
                manifest,
                request.VerifierProfileId,
                electionRecord,
            ...
            results.AddRange(await CheckExternalReviewEvidenceAsync(
                ...
                electionRecord,
                cancellationToken));
            results.AddRange(await CheckOperationalSecurityEvidenceAsync(
                request.PackagePath,
                manifest,
                request.VerifierProfileId,
                electionRecord,
                cancellationToken));
            results.AddRange(await CheckRegulatoryClaimEvidenceAsync(
                request.PackagePath,
                manifest,
                cancellationToken));
